        ...
        for i in cursponer :
            if i['handler'] == '王晓英':
                if i['name'] == '信研':
                    ...
                    if countnum.find_one({'name':i['name'],'handler':'王晓英'})['countnum'] > i['acc_number']:
                        pass
                    else:
                        wxynamelist.append(i['name'])
        itchat.send('{0}以上成员开始圈米中....'.format(wxynamelist), toUserName='{0}'.format(fromusername))
        mainaccount = 'bjxyw001'
        handler = '王晓英'
        quanmi(wxynamelist,fromusername,mainaccount,handler)
    elif msg['Text'] == '刘玉芬团队圈米':
        fromusername = msg['FromUserName']
        lyfnamelist = []
        for i in cursponer :
            if i['handler'] == '刘玉芬':
                if countnum.find_one({'name':i['name'],'handler':'刘玉芬'})['countnum'] > i['acc_number']:
                    pass
                else:
                    lyfnamelist.append(i['name'])
        itchat.send('{0}以上成员开始圈米中....'.format(lyfnamelist), toUserName='{0}'.format(fromusername))
        mainaccount = 'bjlyf001'
        handler = '刘玉芬'
        quanmi(lyfnamelist,fromusername,mainaccount,handler)
    elif msg['Text'] == '江国华团队圈米':
        fromusername = msg['FromUserName']
        jghnamelist=[]
        for i in cursponer :
            if i['handler'] == '江国华':
                if countnum.find_one({'name':i['name'],'handler':'江国华'})['countnum'] > i['acc_number']:
                    pass
                else:
                    jghnamelist.append(i['name'])
        itchat.send('{0}以上成员开始圈米中....'.format(jghnamelist), toUserName='{0}'.format(fromusername))
        mainaccount = 'bjjghua001'
        handler = '江国华'
        quanmi(jghnamelist,fromusername,mainaccount,handler)
    elif msg['Text'] == '王少博团队圈米':
        fromusername = msg['FromUserName']
        wsbnamelist=[]
        for i in cursponer :
            if i['handler'] == '王少博':
                if countnum.find_one({'name':i['name'],'handler':'王少博'})['countnum'] > i['acc_number']:
                    pass
                else:
                    wsbnamelist.append(i['name'])
        itchat.send('{0}以上成员开始圈米中....'.format(wsbnamelist), toUserName='{0}'.format(fromusername))
        mainaccount = 'bjyh001'
        handler = '王少博'
        quanmi(wsbnamelist,fromusername,mainaccount,handler)
    elif msg['Text'] == '陈连萍团队圈米':
        fromusername = msg['FromUserName']
        wsbnamelist=[]
        for i in cursponer :
            if i['handler'] == '陈连萍':
                if countnum.find_one({'name':i['name'],'handler':'陈连萍'})['countnum'] > i['acc_number']:
                    pass
                else:
                    wsbnamelist.append(i['name'])
        itchat.send('{0}以上成员开始圈米中....'.format(wsbnamelist), toUserName='{0}'.format(fromusername))
        ...
        for i in cursponer :
            if i['handler'] == '刘勇豪':
                if i['name'] == '刘勇豪':
                    ...
                    if countnum.find_one({'name': i['name'], 'handler': '刘勇豪'})['countnum'] > i['acc_number']:
                        pass
                    else:
                        lyhnamelist.append(i['name'])
        itchat.send('{0}以上成员开始圈米中....'.format(lyhnamelist), toUserName='{0}'.format(fromusername))
        mainaccount = 'bjyh001'
        handler = '刘勇豪'
        quanmi(lyhnamelist,fromusername,mainaccount,handler)
    else:
        pass
#获得所需串米人的资
def get_ziliao(name):
...
#钱数汇总
def allthemoney(datetime,namelist,fromusername):
    cur = cashnum.find()
    price = 0
    ...
    itchat.send("{0}{1}团队总共需要发人民币：{2}元".format(datetime,handler,price), toUserName='{0}'.format(fromusername))
...
#提现格式计算
def tixian(missUSD,rewardUSD,name,fromusername):
    print(datetime)
    missRMB = (float(missUSD)) * 6.8
    rewardRMB = float(rewardUSD) * 6.8
    allUSD = float(missUSD) + float(rewardUSD)
    print(name)
    itchat.send("{8} \n{0} \n🌹:({1})＄*6.8={2}元 \n🌻:{3}＄*6.8={4}元 \n总计:{5}元 \n({6}美金)×6.8= ({7}) 人民币".format(name,missUSD,int(missRMB),rewardUSD,int(rewardRMB),int(missRMB + rewardRMB),allUSD,int(missRMB + rewardRMB),datetime), toUserName='{0}'.format(fromusername))
    print("🌹:(" + str(missUSD)+")＄*6.8=" + str(int(missRMB)) + "元")
    print("🌻:" + str(rewardUSD) + "＄*6.8=" + str(int(rewardRMB)) + "元")
    print("总计:{0}元".format(str(int(missRMB + rewardRMB))))
    print("({0}美金)×6.8=({1})人民币".format(str(allUSD),str(int(missRMB + rewardRMB))))
    return int(missRMB + rewardRMB)

def quanmi(namelist,fromusername,mainaccount,handler):
    for name in namelist:
        ...
        itchat.send("{0}的账户一共有{1}个".format(name,maxnum), toUserName=fromusername)
        while i <= maxnum:
            wrong = 1
            curse = 1
            if i >= 9:
            ...
            try:
                commissions = wd.find_element_by_xpath('//div[@class="col-md-3 col-sm-6"][2]/div/div/p').get_attribute('data-balance')  # 获取动态钱包钱数
                ...
                try:
                    ...
                    rewards = wd.find_element_by_xpath('//div[@class="col-md-3 col-sm-6"][4]/div/div/p').get_attribute(
                        ...
                        try:
                            ...
                            itchat.send("密码或者账号错误储存的账户为：{0}，密码为：{1} \n开始下一个账户".format(account, pw),
            ...
            while intcommissions > 0 or intrewards > 0:
                ...
                if intcommissions > 0 and intrewards == 0 :
                    Select(wd.find_element_by_id("partition_transfer_partition_user_wallet_id")).select_by_visible_text('commissions wallet ${0}'.format(commissions))
                    ...
                    try:
                        Select(wd.find_element_by_id("partition_transfer_partition_user_wallet_id")).select_by_visible_text("rewards wallet ${0}".format(rewards))
                    except Exception as e:
                        wd.refresh()
            ...
            if curse == 1:
                ...
                if intcommissions == 0 and intrewards == 0:
                    ...
                    wd.quit()
            if i == maxnum:
                sigleprice = tixian(comm, re, name,fromusername)
    ...
    if len(namelist) >0:
        ...
        for i in countcur:
            if i['handler'] == handler:
                countnum.update({'handler': handler}, {'$set': {'countnum': 0}})
    namelist = []
